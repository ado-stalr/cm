...
struct KunnData
{
	...
	std::vector<int> matchingPair; //Согласованые пары
	std::vector<bool> usedPair;    //Использованные пары
};
...
std::optional<Args> ParseArgs(int argc, char* argv[])
{
	if (argc != 2)
	{
		...
		std::cout << "Usage: lw4_2.exe <matrix fileName>\n";
		return std::nullopt;
	}
...
int ReadMatrixFromStream(std::ifstream& input, Matrix& field, int sizeX, int sizeY)
{
...
int main(int argc, char* argv[])
{
	...
	ReadMatrixFromStream(input, matrix, q, p);
